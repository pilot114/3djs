...
function Core(config) {
    ...
    this.playerUpdate = (delta) => {
        ...
        this.player.velocity.y -= this.player.gravity * this.player.mass;
        ...
        if (this.player.isRun && !this.player.isDuck) {
            this.player.velocity.z *= this.player.runFactor;
        }
        // присесть
        if (this.player.isDuck) {
            this.player.velocity.z *= this.player.duckFactor;
            this.player.velocity.x *= this.player.duckFactor;
        }
        ...
        // простая проверка на поверхность. TODO
        ...
        // должно идти после проверки на поверхность
        if (this.player.isDuck) {
     ...
     */
    this.pick = () => {
        // райкастером можно сделать псевдодетект коллизий  - если объект слишком близко, мы не можем на него идти
        // НО ЭТО БУДЕТ РАБОТАТЬ ТОЛЬКО ПОКА СМОТРИШЬ НА ОБЪЕКТ))
